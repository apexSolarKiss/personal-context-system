...
**This release:** `v0.8.0` — artifact roles, optional plane separation, and a structural public/private wall (minor; backward-compatible — systems generated from `v0.7.0` and earlier remain valid; this release requires no migration). This tagged release consolidates three user-facing changes since `v0.7.0`. The three **artifact roles** are named explicitly. A flat system remains the default and needs no justification; plane separation is an **opt-in, instance-owned, reversible** choice when volume earns it. The setup flow's public/private rule becomes **structural**: the generated private destination is never placed inside the scaffold clone, and the previous explicit-override path into the clone is removed. The wall's own description is also corrected — folder separation defines the boundary and makes it auditable, while a tool *cannot* reach past it only where that tool is actually confined to the path-scoped grant. Maintainer-side, the repository installed its resolved execution-protocol carrier and re-synced it four times; that governs work on this scaffold only and is not inherited by generated private systems. `templates/` and the self-contained `SETUP-PROMPT.md` (with its inlined Template Appendix) remain reconciled. Release closure creates the annotated `v0.8.0` tag on this commit and fast-forwards `stable` to the same commit; pin to a version tag explicitly for reproducible use.
