...
**This release:** `v0.5.1` — two small, optional usability nudges (no new files, no breaking changes). **Import existing context:** setup can offer to fold in context you already have in another AI tool, an old chat, a memory export, or a document — it generates a short extraction prompt on the spot (one or two high-signal sources) and treats the result as *draft* material you review before it becomes canonical. **Global behavior preferences:** setup can generate a short behavior-only block — derived from `identity-and-voice.md`, no life facts — to paste into your tool's always-on custom-instructions/preferences area, so it treats you consistently outside a single project. The self-contained `SETUP-PROMPT.md` upload/paste flow remains the universal floor. Lands in `SETUP-PROMPT.md` + the ADR and owner-manual templates (and their appendix mirrors). The `v0.5.1` tag is cut on this release's merge commit and `stable` is fast-forwarded to it; pin to a version tag explicitly for reproducible use.
