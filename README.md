...
**This release:** `v0.6.0` — bootstrap-only connector mount (minor; the no-connector upload floor and filesystem-direct paths are unchanged and fully supported). In healthy cloud-connector mode the AI project now keeps only `_BOOTSTRAP.md` as its standing mount; the bootstrap carries the exact `_INDEX.md` locator and the index is fetched live, so a mounted index can no longer silently serve stale routing. Generated `_BOOTSTRAP` gains a required `INDEX_CANONICAL_LOCATOR` field; the read order, the `_INDEX` connector protocol, the ADR (four operating modes), the project instructions, the owner manual, and the self-contained `SETUP-PROMPT.md` (with its inlined Template Appendix) are reconciled to match. The `v0.6.0` tag is created on this release's merge commit and `stable` is fast-forwarded to it — which also advances `stable` past the interim commits landed after `v0.5.3` (e.g. the SHARE-KIT wording change), so the update is not literally a one-feature `stable` delta; pin to a version tag explicitly for reproducible use.
