...
**This release:** `v0.5.2` — a first-contact setup-reliability fix (no new files, no breaking changes). **Batch delivery (chat-only):** the setup guide no longer dribbles the generated files out one-at-a-time with a pause after each — a slow drip that left passive users with a half-built system. It now delivers the **complete required bundle** at once: a downloadable zip / individual `.md` files where the tool supports them, otherwise the full Markdown for every file emitted together (or split into self-continuing `PART 1 of N` parts, never pausing for the user between files). **Delivery audit:** before install it marks each required output — `identity-and-voice.md`, the domain files, `_BOOTSTRAP.md`, `_INDEX.md`, `context-architecture-decisions.md`, `how-to-use-this-system.md`, the project-instructions paste-in — as delivered or MISSING, and generates any MISSING file before continuing (tied to emitted artifacts, not to "done" wording). **Cloud-save honesty:** an *offer* to save to Google Drive / Dropbox / cloud is not evidence of write capability — the guide may claim a save only after an actual save returns a visible file, path, or link. The self-contained `SETUP-PROMPT.md` upload/paste flow remains the universal floor. Lands in `SETUP-PROMPT.md` (setup flow); the inlined templates are unchanged except the mirrored ADR provenance-version example. The `v0.5.2` tag is cut on this release's merge commit and `stable` is fast-forwarded to it; pin to a version tag explicitly for reproducible use.
