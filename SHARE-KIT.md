# Share kit // personal-context-system

*A ready-made, copy-paste chat share for sending this to someone. First-touch happens in chat, not on the repo — so this is the real entry point. Send it as **three separate messages** (bubbles), in order, so the prompt in bubble 2 stays cleanly long-press-copyable on its own.*
...
```
Your AI memory shouldn't be locked to whatever product happens to render it — ChatGPT, Claude, Gemini, or whatever comes next.
...
*Verbatim copy of `BOOTSTRAP-PROMPT.md`. Keep the two in sync; if one changes, change both. The URL points at the `stable` alias and carries no version or template count, so this share never rots — send it once, it keeps working as the system evolves.*
...
Do not read the GitHub repo tree. Do not continue unless you can see (1) the setup flow and (2) the Template Appendix with all its template sections.

If you cannot open URLs, say exactly: "I can't open URLs from here, so this bootstrap cannot continue. To move forward, please upload or paste the full SETUP-PROMPT.md file, including the Template Appendix."
```
